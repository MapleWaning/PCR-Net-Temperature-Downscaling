...
def parse_args():
    ...
    parser = argparse.ArgumentParser(description="Train the PCR-Net ablation without the terrain attention gate.")
    configure_common_parser(parser, config)
    parser.set_defaults(generalization="year", use_attention=False, use_sft=True)
...
def main():
    ...
    train_guided_ablation(args, config, "no-attention-gate")
