...
def parse_args():
    ...
    parser = argparse.ArgumentParser(description="Train the spatial PCR-Net ablation without attention and SFT.")
    configure_common_parser(parser, config)
    parser.set_defaults(generalization="station", use_attention=False, use_sft=False)
...
def main():
    ...
    train_guided_ablation(args, config, "no-attention-spatial")
